...
class MinHeap:
    ...
    def is_heap(self, parent_pos=0):  # O(n) time
        if parent_pos == -1:
            return True

        left_child = self._left(parent_pos)
        right_child = self._right(parent_pos)

        if left_child != -1 and self.array[parent_pos] > self.array[left_child]:
            return False

        if right_child != -1 and self.array[parent_pos] > self.array[right_child]:
            return False
