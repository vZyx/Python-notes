...
class MinHeap:
	...
	# But we just do all the operations in place and keep utilizing the array
	# So internally, keep poping elements and _heapify_down
	# The elements we pop, we add to the end of the array as our storage
	# now elements are in the array but from large to small ==> reverse
    def sort_heap(self):
