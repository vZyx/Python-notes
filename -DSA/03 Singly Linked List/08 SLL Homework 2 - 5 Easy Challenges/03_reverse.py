...
def test3():
    ...
    repr_before = lst.debug_str_address()
    lst.reverse()
    repr_after = list(reversed(lst.debug_str_address()))

    #lst.debug_print_existing_nodes()

    assert repr_before == repr_after, f'Mismatch between expected=\n[{repr_after}] and result=\n[{repr_before}] in {func_name}'
    print('PASSED\n')
